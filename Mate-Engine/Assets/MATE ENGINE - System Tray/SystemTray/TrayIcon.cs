...
namespace Utils
{
    public static partial class TrayIcon
    {
        ...
        public static void Init(string appName, string tooltip, Texture2D iconTexture, List<(string, Action)> actions = null)
        {
            ...
            {
#if UNITY_EDITOR
                Debug.Log("[SystemTray] System Tray Icon is already initialized for this session. Reusing active connection without pipeline interruption.");
#else
                Debug.LogWarning("[SystemTray] Init called while Tray Icon is already active. Reusing active instance.");
#endif
                return;
            }
        ...
        private static bool CreateMessageWindow()
        {
            ...
            if (classAtom == 0)
            {
                ...
                if (win32Err != 1410) // 1410 = ERROR_CLASS_ALREADY_EXISTS (normal across Unity Editor Play Mode sessions)
                {
                    Debug.LogError($"RegisterClassEx Failed. Error: {win32Err}");
                    return false;
                }
            }
