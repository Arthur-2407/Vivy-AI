...
namespace LLMUnitySamples
{
    public class ChatBot : MonoBehaviour
    {
        ...
        [Header("LLM")]
        public MonoBehaviour llmCharacter;
        ...
        void Start()
        {
            conversationProvider = llmCharacter as IConversationProvider;
            memoryProvider = llmCharacter as IMemoryProvider;
            llmProvider = llmCharacter as ILLMProvider;

            if (conversationProvider == null)
            {
                var provider = FindFirstObjectByType<VivyConversationProvider>();
            ...
            ShowLoadedMessages();
            if (llmProvider != null)
                _ = llmProvider.Warmup(WarmUpCallback);
            else
                WarmUpCallback();
            FindAvatarSmart();
        }
        ...
        void ShowLoadedMessages()
        {
            ...
            var chatHistory = memoryProvider.GetChatHistory();
            int start = 1;
            int total = chatHistory.Count;
            if (maxMessages > 0)
                start = Mathf.Max(1, total - maxMessages);

            for (int i = start; i < total; i++)
            {
                AddBubble(chatHistory[i].content, i % 2 == 1);
            }
            StartCoroutine(ScrollToBottomNextFrame());
        }
        ...
        void onInputFieldSubmit(string newText)
        {
            ...
            Task chatTask = conversationProvider != null ? conversationProvider.Chat(
                message,
        ...
        {
            if (llmProvider != null) llmProvider.CancelRequests();
            AllowInput();
        }
        ...
        void OnValidate()
        {
            ...
            if (onValidateWarning && llmCharacter != null && llmCharacter is LLMCharacter character)
            {
                if (!character.remote && character.llm != null && character.llm.model == "")
                {
                    Debug.LogWarning($"Please select a model in the {character.llm.gameObject.name} GameObject!");
