...
namespace UniGLTF
{
    ...
    public class MaterialFactory : IResponsibilityForDestroyObjects
    {
        ...
        public async Task<Material> LoadAsync(MaterialDescriptor matDesc, GetTextureAsyncFunc getTexture, IAwaitCaller awaitCaller)
        {
            ...
            {
                if (matDesc.TextureSlots.ContainsKey(kv.Key)) continue;
                material.SetColor(kv.Key, kv.Value);
            }

            foreach (var kv in matDesc.Vectors)
            {
                if (matDesc.TextureSlots.ContainsKey(kv.Key) || (kv.Key == "_MainTex" && material.HasProperty("_MainTex_ST")))
                {
                    if (material.HasProperty(kv.Key + "_ST"))
                    {
                        material.SetVector(kv.Key + "_ST", kv.Value);
                    }
                    else if (material.HasProperty(kv.Key) && !matDesc.TextureSlots.ContainsKey(kv.Key))
                    {
                        material.SetTextureScale(kv.Key, new Vector2(kv.Value.x, kv.Value.y));
                        material.SetTextureOffset(kv.Key, new Vector2(kv.Value.z, kv.Value.w));
                    }
                    continue;
                }
