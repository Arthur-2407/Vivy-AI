...
public class SettingsHandlerDropdowns : MonoBehaviour
{
    ...
    public MonoBehaviour llmComponent;
    private Vivy.Communication.ILLMProvider llm;

    private readonly int[] contextOptions = { 2048, 4096, 8192, 16384, 32768 };

    void Start()
    {
        llm = llmComponent as Vivy.Communication.ILLMProvider;
        if (llm == null) llm = FindFirstObjectByType<Vivy.Communication.VivyConversationProvider>();
