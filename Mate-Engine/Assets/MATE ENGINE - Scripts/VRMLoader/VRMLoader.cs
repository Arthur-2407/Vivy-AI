...
public class VRMLoader : MonoBehaviour
{
    ...
    /// <summary>
    /// Attempts to clean up missing-script component slots on a runtime-instantiated
    /// avatar model (e.g. from a modded .me asset bundle). At runtime, Unity's
    /// GetComponents&lt;Component&gt;() returns null entries for missing-script slots.
    /// We log and suppress these gracefully. The actual serialized data cleanup is
    /// performed at edit-time by VivyProjectAutoRemediation.
    /// </summary>
    private void CleanMissingScripts(GameObject root)
    {
        if (root == null) return;
        int cleaned = 0;
        ...
        foreach (var t in transforms)
        {
            ...
            {
                // Unity returns null for missing-script components.
                // At runtime, we cannot remove them via API, but we can prevent NullRef
                // exceptions by checking here. The warning is suppressed by the fact that
                // Unity only logs it once per component slot at load time.
                if (comp == null)
                {
                    cleaned++;
                }
        ...
        if (cleaned > 0)
        {
            Debug.Log($"[VRMLoader] Detected {cleaned} missing-script component slot(s) in loaded avatar. " +
                      "These are from scripts in the mod bundle that don't exist in this runtime. " +
                      "Use Vivy AI > Run Complete Project Auto-Remediation in Editor to clean prefabs.");
        }
    }
