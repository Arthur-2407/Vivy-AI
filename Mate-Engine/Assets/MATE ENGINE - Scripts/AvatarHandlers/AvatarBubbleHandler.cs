...
public class AvatarBubbleHandler : MonoBehaviour
{
    ...
    void Update()
    {
        if (!Application.isPlaying || animator == null || !animator.IsValidAndPlaying() || attachTarget == null) return;
        if (bone == null)
    ...
    {
        return animator != null && animator.IsValidAndPlaying() && animator.GetBool("isDragging");
    }
