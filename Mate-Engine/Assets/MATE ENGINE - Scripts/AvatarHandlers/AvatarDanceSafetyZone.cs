...
{
    public Camera cam;
    public UniWindowController uniWindow;
    ...
    {
        if (!wasActive) return;
        if (cam == null || hip == null || uniWindow == null) return;
#if !UNITY_EDITOR
        if (Screen.fullScreen) return;
#endif
        if (uniWindow.shouldFitMonitor || uniWindow.isZoomed) return;
        ...
        if (moveWindowAlong && !uniWindow.shouldFitMonitor && !uniWindow.isZoomed)
        {
            var wp = uniWindow.windowPosition;
            wp.x += stepPx;
            uniWindow.windowPosition = wp;
        }
    }
    ...
    void RestoreCamera()
    {
        ...
        _returnCoroutine = StartCoroutine(SmoothReturnToCenterRoutine());
    }

    private System.Collections.IEnumerator SmoothReturnToCenterRoutine()
    {
        Vector3 startCamPos = cam.transform.position;
        Quaternion startCamRot = cam.transform.rotation;
        
        Transform root = ResolveCurrentModelRoot();
        Vector3 startRootPos = (root != null) ? root.localPosition : Vector3.zero;
        Quaternion startRootRot = (root != null) ? root.localRotation : Quaternion.identity;
        ...
        while (elapsed < duration)
        {
            ...
            if (cam != null)
            {
                cam.transform.position = Vector3.Lerp(startCamPos, camPosBeforeDance, t);
                cam.transform.rotation = Quaternion.Slerp(startCamRot, camRotBeforeDance, t);
            }

            if (root != null && (root.localPosition.sqrMagnitude > 0.0001f || root.localRotation != Quaternion.identity))
            {
                root.localPosition = Vector3.Lerp(startRootPos, Vector3.zero, t);
                root.localRotation = Quaternion.Slerp(startRootRot, Quaternion.identity, t);
            }
