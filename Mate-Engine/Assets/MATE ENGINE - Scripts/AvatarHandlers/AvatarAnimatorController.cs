...
public class AvatarAnimatorController : MonoBehaviour
{
    ...
    [HideInInspector] public bool forceDanceOverride = false;
    ...
    void OnEnable()
    {
        ...
        // Only apply gender params if the animator already has a valid RuntimeAnimatorController.
        // During VRM loading the controller may not be assigned yet, causing the
        // "Animator is not playing an AnimatorController" warning. Update() re-applies these
        // every frame anyway, so no data is lost by skipping here.
        if (IsAnimatorValid())
        {
    ...
    void CheckForSound()
    {
        if (forceDanceOverride)
        {
            if (!isDancing) StartDancing();
            return;
        }
    ...
    {
        isDancing = value;
        if (IsAnimatorValid()) animator.SetBool(isDancingParam, value);
    ...
    public void SetDancingOverride(bool danceState)
    {
        forceDanceOverride = danceState;
        enableDancing = danceState;
        if (danceState)
        {
            if (!isDancing) StartDancing();
        }
    ...
    [Header("Circadian Modulation")]
    public float currentCircadianEnergy = 1.0f;
    public string currentCircadianPhase = "Afternoon";

    public void SetCircadianEnergy(float energy, string phase)
    {
        currentCircadianEnergy = Mathf.Clamp01(energy);
        if (!string.IsNullOrEmpty(phase)) currentCircadianPhase = phase;

        // Modulate idle transition speeds based on energy (lower energy = slower idle switches)
        IDLE_SWITCH_TIME = Mathf.Lerp(20f, 8f, currentCircadianEnergy);

        if (animator != null)
        {
            if (AnimatorHasParameter("circadianEnergy", AnimatorControllerParameterType.Float))
            {
                animator.SetFloat("circadianEnergy", currentCircadianEnergy);
            }
        }
    }
    ...
    {
        isDragging = value;
        if (IsAnimatorValid()) animator.SetBool(isDraggingParam, value);
    }
    ...
    void UpdateIdleStatus()
    {
        ...
        if (isIdle != inIdle)
        {
            bool returningToIdle = !isIdle && inIdle;
            isIdle = inIdle;
            animator.SetBool(isIdleParam, isIdle);

            if (returningToIdle && animator.transform != null && animator.transform.localPosition.sqrMagnitude > 0.0001f)
            {
                StartCoroutine(SmoothRecenter(animator.transform));
            }
        }
    }

    IEnumerator SmoothRecenter(Transform modelTransform)
    {
        ...
        while (elapsed < duration)
        {
            ...
            {
                modelTransform.localPosition = Vector3.Lerp(startPos, Vector3.zero, t);
                modelTransform.localRotation = Quaternion.Slerp(startRot, Quaternion.identity, t);
            }
            yield return null;
        }
        if (modelTransform != null)
