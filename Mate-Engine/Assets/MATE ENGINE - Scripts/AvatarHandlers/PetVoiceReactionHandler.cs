...
public class PetVoiceReactionHandler : MonoBehaviour
{
    ...
    void TrySetup()
    {
        if (!avatarAnimator || !avatarAnimator.IsValidAndPlaying()) return;
    ...
    {
        if (!hasSetup) TrySetup();
        if (cachedCamera == null || avatarAnimator == null || !avatarAnimator.IsValidAndPlaying()) return;
    ...
    {
        if (string.IsNullOrEmpty(layerName) || avatarAnimator == null || !avatarAnimator.IsValidAndPlaying()) return 0;
    ...
    void OnDrawGizmos()
    {
        if (!showDebugGizmos || !Application.isPlaying || !cachedCamera || !avatarAnimator || !avatarAnimator.IsValidAndPlaying()) return;
        for (int i = 0; i < regions.Count; i++)
        {
            var region = regions[i];
