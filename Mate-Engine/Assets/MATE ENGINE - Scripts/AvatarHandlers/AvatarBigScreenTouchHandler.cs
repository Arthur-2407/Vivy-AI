...
public class AvatarBigScreenTouchHandler : MonoBehaviour
{
    ...
    void Update()
    {
        if (bigScreenHandler == null || avatarAnimator == null || !avatarAnimator.IsValidAndPlaying() || mainCamera == null)
            return;
    ...
    void OnDisable()
    {
        if (avatarAnimator != null && avatarAnimator.IsValidAndPlaying() && hasHairStrokeParam) avatarAnimator.SetBool(HairStrokeHash, false);
        CleanupMouseCollider();
    }
    ...
    void HandleSpringBoneTouch()
    {
        ...
        // ColliderObjekt an Mausposition setzen (auf 3D-Position in Avatarnhe)
