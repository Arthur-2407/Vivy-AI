...
public class AISystemPromptBinder : MonoBehaviour
{
    ...
    private Vivy.Communication.ILLMProvider target;
    ...
    void Reset()
    {
        ...
        if (!targetComponent) targetComponent = FindFirstObjectByType<Vivy.Communication.VivyConversationProvider>();
    }
    ...
    void Awake()
    {
        ...
        target = targetComponent as Vivy.Communication.ILLMProvider;
        if (target == null) target = FindFirstObjectByType<Vivy.Communication.VivyConversationProvider>();

        string path = GetFixedPromptPath();
        string txt = target != null ? target.Prompt : "";
